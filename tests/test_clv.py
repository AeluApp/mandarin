...
def _make_clv_db():
    """Create in-memory DB with tables needed by the CLV module."""
...
class TestComputeCohortRetention(unittest.TestCase):
    ...
    def test_two_users_same_cohort(self):
        ...
        self.conn.executescript("""
            ...
            VALUES (1, 'a@test.com', '2025-06-01 10:00:00');
            INSERT INTO user (id, email, created_at)
            VALUES (2, 'b@test.com', '2025-06-15 12:00:00');

            -- User 1 has a session on day 1 after signup
            INSERT INTO session_log (user_id, started_at, completed, duration_seconds)
            VALUES (1, '2025-06-02 10:00:00', 1, 600);

            -- User 2 has a session on day 1 after signup
            INSERT INTO session_log (user_id, started_at, completed, duration_seconds)
            VALUES (2, '2025-06-16 12:00:00', 1, 300);
        """)
    ...
    def test_empty_tables(self):
        """Empty user table returns empty list."""
    ...
    def test_users_no_sessions(self):
        ...
        self.conn.execute(
            "INSERT INTO user (id, email, created_at) "
            "VALUES (1, 'solo@test.com', '2025-07-01 00:00:00')"
        )
        results = compute_cohort_retention(self.conn)
        self.assertEqual(len(results), 1)
        self.assertEqual(results[0]["users"], 1)
        self.assertEqual(results[0]["day_1"], 0.0)
...
class TestComputeLtv(unittest.TestCase):
    ...
    def test_individual_ltv_with_payments(self):
        ...
        self.conn.executescript("""
            INSERT INTO user (id, email, created_at)
            VALUES (1, 'payer@test.com', '2025-01-01 00:00:00');
    ...
    def test_aggregate_ltv(self):
        ...
        self.conn.executescript("""
            INSERT INTO user (id, email, created_at)
            VALUES (1, 'u1@test.com', '2025-01-01');
            INSERT INTO user (id, email, created_at)
            VALUES (2, 'u2@test.com', '2025-01-01');
...
class TestPredictLtvSegment(unittest.TestCase):
    ...
    def test_high_engagement_user(self):
        ...
        self.conn.execute(
            "INSERT INTO user (id, email, created_at) "
            "VALUES (1, 'power@test.com', '2025-04-01 00:00:00')"
        )
    ...
    def test_low_engagement_user(self):
        ...
        self.conn.execute(
            "INSERT INTO user (id, email, created_at) "
            "VALUES (1, 'casual@test.com', '2025-04-01 00:00:00')"
        )
        self.conn.execute(
            "INSERT INTO session_log (user_id, started_at, completed, duration_seconds) "
            "VALUES (1, '2025-04-02 10:00:00', 0, 60)"
        )
        self.conn.commit()
    ...
    def test_medium_engagement_user(self):
        ...
        self.conn.execute(
            "INSERT INTO user (id, email, created_at) "
            "VALUES (1, 'mid@test.com', '2025-04-01 00:00:00')"
        )
        # 3 sessions, 2/3 completed (67% < 70%), 200s avg
        for i, (comp, dur) in enumerate([(1, 200), (1, 200), (0, 200)]):
            self.conn.execute(
                "INSERT INTO session_log (user_id, started_at, completed, duration_seconds) "
                "VALUES (1, ?, ?, ?)",
    ...
    def test_no_sessions_returns_unknown(self):
        ...
        self.conn.execute(
            "INSERT INTO user (id, email, created_at) "
            "VALUES (1, 'ghost@test.com', '2025-04-01 00:00:00')"
        )
        self.conn.commit()
