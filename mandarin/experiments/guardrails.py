...
def _compute_guardrail_metric(
    ...
    if metric == "session_completion_rate":
        ...
        row = conn.execute(sql, user_ids).fetchone()
        ...
        try:
            sql = f"""SELECT COUNT(*) as cnt FROM error_log
                    WHERE session_id IN (
                        SELECT id FROM session_log WHERE user_id IN ({placeholders})
                    ) AND error_type LIKE '%crash%'"""
            row = conn.execute(sql, user_ids).fetchone()
            crashes = row["cnt"] or 0
            sql = f"SELECT COUNT(*) as cnt FROM session_log WHERE user_id IN ({placeholders})"
            session_row = conn.execute(sql, user_ids).fetchone()
        ...
        try:
            sql = f"""SELECT AVG(gap) as avg_gap FROM (
