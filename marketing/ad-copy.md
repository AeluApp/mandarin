# Ad Copy Variants — Ready to Paste

## Google Search Ads

### Campaign 1: HSK Study Intent
**Keywords:** hsk study app, hsk exam preparation, hsk 3 study plan, hsk learning app
**Headline 1:** Learn Chinese with Adaptive SRS
**Headline 2:** HSK 1-6 Aligned Curriculum
**Headline 3:** Free to Start — No Credit Card
**Description 1:** 27 drill types that adapt to your weak spots. Track your HSK readiness with real projections. Built by a learner, not a corporation.
**Description 2:** Stop drilling random vocab. Aelu tracks what you actually struggle with and builds your next session around it. HSK 1-6 coverage.
**Display URL:** aeluapp.com/hsk

### Campaign 2: Anki Alternative
**Keywords:** anki alternative chinese, chinese flashcard app, srs chinese app, best spaced repetition chinese
**Headline 1:** Tired of Building Anki Decks?
**Headline 2:** Chinese SRS That Builds Itself
**Headline 3:** 27 Drill Types, Not Just Flashcards
**Description 1:** Aelu goes beyond flashcards — reading, listening, speaking, and tone drills that adapt to your level. No deck-building required.
**Description 2:** Like Anki but with a curriculum. Graded reading, listening practice, and drills that target your actual gaps. Free tier available.
**Display URL:** aeluapp.com/srs

### Campaign 3: Serious Learner
**Keywords:** best app learn mandarin, chinese learning app advanced, learn chinese seriously, mandarin app for adults
**Headline 1:** Chinese Learning for Adults
**Headline 2:** No Cartoons. No Streaks. Real Progress.
**Headline 3:** Read Real Chinese from Day One
**Description 1:** Aelu is built for serious learners. Graded reading with instant glosses, adaptive drills, and HSK projections. Not gamified — effective.
**Description 2:** Every word you look up during reading becomes a drill. The cleanup loop turns real exposure into focused practice. Free to try.
**Display URL:** aeluapp.com

---

## Meta (Facebook/Instagram) Ads

### Ad 1: The Cleanup Loop (Story/Reel format)
**Primary text:** Most Chinese apps drill random vocabulary. Aelu drills the words you actually struggled with.

Read a passage. Tap the words you don't know. Your next session focuses on exactly those words.

It's called the cleanup loop. And it's how serious learners actually improve.

Free to start at aeluapp.com
**Headline:** Learn Chinese the way your brain works
**CTA:** Sign Up

### Ad 2: Anti-Duolingo (Feed format)
**Primary text:** I spent two years on Duolingo Chinese and couldn't read a restaurant menu.

So I built something different. 27 drill types. Graded reading with real Chinese text. Listening practice at adjustable speed. An adaptive system that learns what I need to work on.

No cartoon owl. No streak anxiety. Just effective learning.

Free to start.
**Headline:** Chinese learning for adults
**CTA:** Learn More

### Ad 3: HSK Exam Prep (Feed format)
**Primary text:** HSK exam in 3 months? Here's what most study apps won't tell you:

You don't need more flashcards. You need targeted practice on your actual weak spots.

Aelu tracks your mastery across all HSK levels and tells you exactly where you stand. Tone drills. Listening comprehension. Character recognition. All in one place.

Free for HSK 1-2. Full access $14.99/month.
**Headline:** Know exactly where you stand for HSK
**CTA:** Sign Up

### Ad 4: The Builder Story (Feed format)
**Primary text:** I'm learning Chinese and I built my own app to do it.

Not because existing apps are bad — but because none of them worked the way I actually learn. I wanted:

- Drills that adapt to what I get wrong
- Reading practice where I can tap any word
- Every word I look up to become a drill
- No AI-generated content with wrong tones

So I built it. And now I'm sharing it.

Free to start at aeluapp.com
**Headline:** Built by a learner, for learners
**CTA:** Sign Up

---

## Reddit Post Templates

### r/ChineseLanguage — Launch Post
**Title:** I built a Chinese learning app focused on the "cleanup loop" — read real Chinese, and every word you look up becomes a drill

**Body:**
Hey everyone. I've been learning Chinese for [X months] and got frustrated with the gap between studying flashcards and actually reading/listening to Chinese.

So I built an app around what I'm calling the "cleanup loop":

1. Read a graded passage or listen to a clip
2. Tap any word you don't know — you get instant pinyin + translation
3. Every word you looked up gets added to your next drill session

The idea is that instead of drilling a generic word list, you're drilling the specific words YOU struggled with in context.

Other features:
- 27 drill types (not just flashcards)
- HSK 1-6 aligned curriculum
- Adaptive scheduling — focuses on your weak spots
- Tone drills with grading
- No AI content generation — all content is human-reviewed

It's free for HSK 1-2 content. Full access is $14.99/month.

Would love feedback from other learners. What would make this more useful to you?

[link]

### r/ChineseLanguage — Value Post (not promotional)
**Title:** The study method that actually helped me retain characters: drilling words I looked up during reading

**Body:**
I've been experimenting with something that's working really well for me and wanted to share.

Instead of studying word lists or random Anki decks, I started tracking every word I had to look up while reading graded Chinese content. Then I made those specific words the focus of my next study session.

The logic is simple: if I needed to look up 看见 while reading a story, that's a genuine gap — not a theoretical one from a textbook. Drilling it after encountering it in context makes it stick way better than drilling it in isolation.

I've been doing this for [X weeks] and my reading fluency has noticeably improved. The words I drill are words I've already seen in context, so the association is stronger.

Anyone else do something similar? Curious how others bridge the gap between "studying" and "actually reading."

### r/LearnChinese — HSK Value Post
**Title:** My system for tracking HSK readiness (beyond just counting words)

**Body:**
A lot of HSK prep advice is just "learn the word list." But passing HSK isn't about knowing 600 words in isolation — it's about listening comprehension, reading speed, and being able to use grammar patterns in context.

I've been tracking my readiness across multiple dimensions:
- Vocabulary recognition rate (not just "have I seen this word")
- Listening comprehension at natural speed
- Tone accuracy (this killed me on HSK 3)
- Grammar pattern usage in context

The biggest surprise: my vocabulary was at HSK 4 level but my listening was barely HSK 2. Focusing on listening drills specifically closed that gap faster than general study.

What dimensions do you track for HSK prep?

---

## Email Sequence (Post-Signup)

### Email 1: Welcome (Immediate)
**Subject:** You're in. Here's how to start.
**Body:**
Welcome to Aelu.

Here's the fastest way to get value:

1. Start a session — the system will assess your level automatically
2. After your first session, check your HSK readiness projection
3. Try the graded reader — tap any word you don't know

Every word you look up during reading becomes a drill in your next session. That's the cleanup loop, and it's the core of how Aelu works.

Your first session takes about 10 minutes.

[Start now →]

### Email 2: Day 3 — The Cleanup Loop
**Subject:** The one feature that changes how you study
**Body:**
Most Chinese apps drill you on a fixed word list. Aelu drills you on the words you actually struggle with.

Here's how:
- Open the Reader and pick a passage at your level
- Read through it. Tap any word you need help with.
- Start a drill session afterward.

You'll notice the words you just looked up showing up in your drills. That's not a coincidence — the system prioritizes words you encountered in real reading.

This is how serious learners study. Not by grinding random lists, but by turning real exposure into focused practice.

[Open the Reader →]

### Email 3: Day 7 — Progress Check
**Subject:** Your first week in numbers
**Body:**
You've been learning for a week. Here's where you stand:

[Dynamic: insert user stats if available, or generic encouragement]

One thing to try this week: the listening practice. Pick a passage you've already read, then listen to it without the text. See how much you catch. Then reveal the transcript and tap the words you missed.

Those missed words go straight into your drill queue.

[Start listening practice →]

### Email 4: Day 14 — Upgrade Nudge (only if on free tier)
**Subject:** Ready for HSK 3?
**Body:**
You've been using Aelu for two weeks. If you're finding value in the adaptive drills and the cleanup loop, the full version unlocks:

- HSK 3-6 curriculum (4,700+ additional words)
- All 27 drill types (free tier has 5)
- Speaking drills with tone grading
- Advanced listening and reading passages
- Full HSK projection and diagnostics

$14.99/month. Cancel anytime.

[Upgrade →]

---

## Budget Recommendations

### Phase 1: Pre-launch (now)
- $0 — build email list organically via Reddit posts and content
- Landing page on GitHub Pages or Netlify (free)
- Formspree free tier for email collection (50 submissions/month)

### Phase 2: Launch (first 30 days)
- $0-100/month — boosted Reddit/Facebook posts only
- Don't run Google Ads yet — need conversion data first
- Focus on organic: 2 Reddit posts/week, 1 blog post/week

### Phase 3: Growth (month 2-6)
- $200-500/month Google Ads on high-intent HSK keywords
- $100-200/month Meta Ads on the "builder story" and "cleanup loop" creatives
- Pause anything with CAC > $30 per paid conversion
- Target: CAC < $15, LTV > $100 (8+ months retention)

### When to STOP paid ads:
- If CAC exceeds $40 per paid user — the unit economics don't work at your price point
- If free-to-paid conversion is below 5% — fix onboarding first
- If churn exceeds 15%/month — fix retention first, ads will just fill a leaky bucket
